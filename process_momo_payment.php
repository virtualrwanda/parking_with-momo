...
// We are using PDO methods for transactions.
...
try {
    ...
    if (isset($payment_result['ref']) && $payment_result['status'] === 'pending') {
        ...
        // Update the parking log with the payment reference using a PDO prepared statement.
        $stmt = $conn->prepare("UPDATE parking_logs SET payment_ref = ?, paid = 0 WHERE id = ?");
        // Bind parameters. The 'paid' column is set to 0 (or false)
        // and will be updated to 1 (or true) by the PayPack webhook.
        $stmt->execute([$ref, $log_id]);
        $stmt->closeCursor();
